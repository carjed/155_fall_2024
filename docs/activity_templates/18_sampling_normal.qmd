---
title: "18. The Normal model & sampling variation"
...
b. Approximate the probability that a car's speed exceeds 60mph.        

```{r}
shaded_normal(mean = 55, sd = 5, a = 60)
```    

c. Provide a *range* for the probability that a car's speed exceeds 67mph: less than 0.0015, between 0.0015 and 0.025, between 0.025 and 0.16, or greater than 0.16.    
...
For example, if I'm traveling 40 miles an hour, my Z-score is -3. That is, my speed is 3 standard deviations *below* the average speed:    
    
```{r}
(40 - 55) / 5
```

a. Consider 2 other drivers. Both drivers are speeding. Who do you think is speeding more?       
...
They are the actual intercept and slope, $\beta_0$ and $\beta_1$, that describe the relationship among the entire population of interest (U.S. counties):

E(pci_2019 | pci_2017) = $\beta_0$ + $\beta_1$ pci_2017 = 1.294 + 1.027 pci_2017

BUT we typically we don't have access to the entire population of interest.
We must **estimate** population parameters using data from a **random sample** of counties.
Below, you'll each take a random sample of 10 U.S. counties.
...
a. Use the `sample_n()` function to take a *random sample* of 2 counties. Reflect:       
    - How do your results compare to your neighbors'?
    - What is the role of `size = 2`?
    - What is the role of `replace = FALSE`? 
...
b. Now, "set the seed" to 155 and re-try your sampling. What changed?
...
```{r}
# DON'T SKIP THIS STEP! Remove the "#" next to set.seed when you're ready
...
Recall the actual **population parameters** describing the relationship between `pci_2019` and `pci_2017`:

E(pci_2019 | pci_2017) = 1.294 + 1.027 pci_2017
...
[Solutions](../solutions/18_sampling_normal.qmd)